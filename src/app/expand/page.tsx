...
const WORKFLOW_ID = 'expand-content-formats'
...
const STEP_LABELS: Record<string, string> = {
  'website-specialist': '🌐 Website Specialist',
  'email-marketing-specialist': '✉️ Email Marketing Specialist',
  'video-director': '🎬 Video Director',
}

export default function ExpandContentFormats() {
  ...
  async function runWorkflow() {
    if (!coreMessage.trim()) { setErrorMsg('กรุณาระบุแก่นข้อความหลัก'); return }
    setLoading(true); setErrorMsg(''); setResult(null)
    try {
      const res = await fetch('/api/workflows/run', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brandSlug, workflowId: WORKFLOW_ID, input: { coreMessage } }),
      })
  ...
  return (
    <main style={{ minHeight: '100vh', background: bg, color: text, fontFamily: "'Inter', 'Sarabun', sans-serif" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: '20px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 18, fontWeight: 800 }}>🧩 Expand Content Formats</div>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px' }}>
        <div style={{ fontSize: 13, color: text2, marginBottom: 20, lineHeight: 1.6 }}>
          Website Specialist, Email Marketing Specialist และ Video Director แปลงแก่นข้อความหลักเดียวกัน
          เป็น 3 รูปแบบพร้อมกัน — ทั้ง 3 ชิ้นจะไปรออนุมัติที่ /approvals เหมือนคอนเทนต์ทั่วไป
        </div>
        ...
        <textarea
          ...
          placeholder="แก่นข้อความหลัก เช่น 'AMOS ช่วยให้ SME ไทยมีทีมการตลาด AI โดยไม่ต้องจ้างคนเพิ่ม'"
          disabled={loading}
          rows={3}
          style={{
        ...
        >{loading ? 'Working…' : 'Expand Content'}</button>
        ...
        {isCompleted && (
          <div style={{ background: `${green}15`, border: `1px solid ${green}55`, borderRadius: 12, padding: 16, textAlign: 'center', fontSize: 13, fontWeight: 600, color: green }}>
            ✓ สร้างครบ 3 รูปแบบแล้ว — ไปที่ <a href="/approvals" style={{ color: green }}>Approvals</a> เพื่อตรวจสอบ
          </div>
        )}
      </div>
    </main>
