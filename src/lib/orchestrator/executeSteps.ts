...
// that walks a WorkflowDefinition's steps, so both entry points stay in sync.
export async function executeSteps(
  { runId, brandId, workflow, brain, workflowInput, priorOutputs, startIndex }: ExecuteParams
  ...
  for (let i = startIndex; i < workflow.steps.length; i++) {
    ...
    try {
      const result = await runAgent(agentDef, agentInput, brain, priorOutputs)
