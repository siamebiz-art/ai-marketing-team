...
import { renderBrandContext } from '@/lib/prompts/brand-context'
import { MODEL_BY_TIER } from './models'
import type { AgentDefinition, AgentRunResult } from './types'

// The only place in this project that calls the Anthropic SDK. Every specialist goes
// through here so caching behavior, model selection, and error handling stay consistent
// without each specialist re-implementing them.
const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY! })

export async function runAgent<TInput, TOutput>(
  def: AgentDefinition<TInput, TOutput>,
  input: TInput,
  brain: MarketingBrainContext,
  ...
  const response = await anthropic.messages.create({
    ...
    system: [
      { type: 'text', text: renderBrandContext(brain), cache_control: { type: 'ephemeral' } },
      { type: 'text', text: def.roleInstructions, cache_control: { type: 'ephemeral' } },
    ],
    messages: [{ role: 'user', content: def.buildUserPrompt(input, brain, priorOutputs) }],
  })
