...
export interface AgentDefinition<TInput = unknown, TOutput = unknown> {
  ...
  modelTier: ModelTier
  maxTokens: number
  // Static (brain-independent) role framing + output-format rules. Kept separate from the
  // brand-context block (which runAgent renders once and prepends as its own cached block)
  ...
  buildUserPrompt: (input: TInput, brain: MarketingBrainContext, priorOutputs: Record<string, unknown>) => string
